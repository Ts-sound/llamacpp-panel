...
class SSHPanel(tk.Frame):
    ...
    def _build_command_preview(self) -> None:
        row = 6
        ttk.Label(self, text="命令:").grid(row=row, column=0, sticky=tk.W, padx=5, pady=3)
        self._cmd_preview = ttk.Label(self, text="", foreground="#555")
        ...
        for var in [self._local_port_var, self._remote_port_var, self._remote_host_var, self._username_var, self._key_file_var]:
    ...
    def _update_cmd_preview(self) -> None:
        ...
        self._cmd_preview.config(text=cmd[:80] + "..." if len(cmd) > 80 else cmd)

    def _on_copy_cmd(self) -> None:
        cmd = self._cmd_preview.cget("text")
